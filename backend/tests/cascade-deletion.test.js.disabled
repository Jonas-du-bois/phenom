/**
 * Tests for observation cascade deletion
 * These tests verify that all associated data is correctly deleted
 */
...
// Mock image service to avoid real Cloudinary calls
...
describe("Observation cascade deletion tests", () => {
  let authToken;
  let adminToken;
  let userId;
  let adminId;
  ...
  describe("User observation deletion", () => {
    beforeEach(async () => {
    ...
    it("devrait supprimer l'observation, ses commentaires et ses images Cloudinary", async () => {
      ...
      expect(imageService.deleteAllImagesForObservation).toHaveBeenCalledWith(
        observationId
      );
    });
  ...
  describe("Suppression de compte utilisateur", () => {
    ...
    it("devrait supprimer le compte et toutes ses observations/commentaires/images", async () => {
      ...
      expect(imageService.deleteAllImagesForObservation).toHaveBeenCalledTimes(
        2
      );
      expect(imageService.deleteAllImagesForObservation).toHaveBeenCalledWith(
        obs1Id
  ...
  afterAll(async () => {
    ...
    await User.deleteMany({
      email: {
        $in: ["cascade@test.com", "cascadeadmin@test.com", "tempuser@test.com"],
      },
    });
