...
describe('Observation Endpoints', () => {
  ...
  describe('GET /api/v1/observations/nearby', () => {
    beforeEach(async () => {
      ...
      await Observation.create([
        ...
        },
        {
          ...
          location: {
            type: 'Point',
            coordinates: [2.3600, 48.8600] // ~3km de Paris
          },
          userId
        },
        {
    ...
    it('should find nearby observations', async () => {
      const response = await request(app)
        .get('/api/v1/observations/nearby?longitude=2.3522&latitude=48.8566&radius=10')
        .expect(200);
    ...
    it('should respect maxDistance parameter', async () => {
      const response = await request(app)
        .get('/api/v1/observations/nearby?longitude=2.3522&latitude=48.8566&radius=1')
        .expect(200);
  ...
  describe('Security and Edge Cases', () => {
    ...
    beforeEach(async () => {
      ...
      const securityAuth = await createAuthenticatedUser('securityuser@example.com');
