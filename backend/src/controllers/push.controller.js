...
class PushController {
  ...
  async subscribe(req, res, next) {
    try {
      ...
      const existing = await PushSubscription.findOne({ userId, 'subscription.endpoint': subscription.endpoint });
      if (existing) {
        existing.subscription = subscription;
        await existing.save();
        return successResponse(res, { id: existing._id }, 'Subscription updated');
      }
  ...
  // Optional: POST /api/v1/push/unsubscribe
  async unsubscribe(req, res, next) {
    try {
      const userId = req.user._id;
      const { endpoint } = req.body;
      if (!endpoint) return errorResponse(res, 'endpoint required', 400);
      await PushSubscription.deleteMany({ userId, 'subscription.endpoint': endpoint });
  ...
  async publicKey(req, res, next) {
    try {
      const key = process.env.VAPID_PUBLIC_KEY || process.env.VITE_VAPID_PUBLIC || '';
