...
/**
 * Validation pour créer une observation (format Phenom Search compatible)
 */
export const createObservationValidation = [
  body('date')
    .trim()
    .notEmpty().withMessage('La date est requise')
    .isLength({ max: 50 }).withMessage('La date ne peut pas dépasser 50 caractères'),

  body('time')
    .optional()
    .trim()
    .isLength({ max: 20 }).withMessage('L\'heure ne peut pas dépasser 20 caractères'),
  ...
  body('credibility')
    .optional()
    .isInt({ min: 0, max: 15 }).withMessage('La crédibilité doit être entre 0 et 15'),

  body('strangeness')
    .optional()
    .isInt({ min: 0, max: 10 }).withMessage('L\'étrangeté doit être entre 0 et 10'),

  body('duration')
    .optional()
    .isInt({ min: 0 }).withMessage('La durée doit être un entier positif (en secondes)'),

  body('locale')
    .optional()
    .isIn(LOCALE_TYPES).withMessage('Type de lieu invalide'),
  ...
  body('observerTypes')
    .optional()
    .isArray().withMessage('observerTypes doit être un tableau')
    .custom((types) => {
      if (!Array.isArray(types)) return false;
      return types.every(t => OBSERVER_TYPES.includes(t));
    }).withMessage(`Types d'observateurs invalides. Valeurs valides: ${OBSERVER_TYPES.join(', ')}`),

  body('ufoShapes')
    .optional()
    .isArray().withMessage('ufoShapes doit être un tableau')
    .custom((shapes) => {
  ...
  body('phenomena')
    .optional()
    .isArray().withMessage('phenomena doit être un tableau')
    .custom((phenom) => {
      if (!Array.isArray(phenom)) return false;
      return phenom.every(p => PHENOMENA.includes(p));
    }).withMessage(`Phénomènes invalides. Valeurs valides: ${PHENOMENA.join(', ')}`),
...
export const updateObservationValidation = [
  ...
  body('location')
    .optional()
    .trim()
    .notEmpty().withMessage('Le lieu ne peut pas être vide')
    .isLength({ max: 200 }).withMessage('Le lieu ne peut pas dépasser 200 caractères'),

  body('country')
    .optional()
    .trim()
    .notEmpty().withMessage('Le pays ne peut pas être vide')
    .isLength({ max: 100 }).withMessage('Le pays ne peut pas dépasser 100 caractères'),
...
export const getObservationsValidation = [
  ...
  query('limit')
    .optional()
    .isInt({ min: 1, max: 500 }).withMessage('La limite doit être entre 1 et 500'),
  ...
  query('minCredibility')
    .optional()
    .isInt({ min: 0, max: 15 }).withMessage('minCredibility doit être entre 0 et 15'),

  query('maxCredibility')
    .optional()
    .isInt({ min: 0, max: 15 }).withMessage('maxCredibility doit être entre 0 et 15'),

  query('minStrangeness')
    .optional()
    .isInt({ min: 0, max: 10 }).withMessage('minStrangeness doit être entre 0 et 10'),

  query('maxStrangeness')
    .optional()
    .isInt({ min: 0, max: 10 }).withMessage('maxStrangeness doit être entre 0 et 10')
];
