/**
 * UFO Observation Seed Data - Complete Phenom Search Format
 * All fields from the Observation model are populated
 *
 * Field descriptions:
 * - date: Observation date (YYYY-MM-DD format)
 * - time: Time of observation (HH:MM format)
 * - location: City/region name in uppercase
 * - country: Country name
 * - state: State/canton/region
 * - description: Detailed witness account (in French)
 * - credibility: Witness credibility score (1-10)
 * - strangeness: How unusual/unexplainable the event is (1-10)
 * - duration: Duration in seconds
 * - locale: Type of location (see Locale types below)
 * - coordinates: GPS coordinates { lat, lng }
 * - observerTypes: Array of observer type codes
 * - ufoShapes: Array of UFO shape codes
 * - phenomena: Array of phenomena codes
 * - tags: Searchable keywords
 * - imageFilename: Associated image file
 * - userIndex: Index of the user who reported this observation
 *
 * Observer types:
 *   GND (Ground), MIL (Military), CIV (Civilian), HQO (High Quality Observer),
 *   SCI (Scientific), CST (Coastguard), SEA (Seafarer), NWS (News/Media)
 *
 * UFO shapes:
 *   SCR (Saucer), CIG (Cigar), DLT (Delta/Triangle), NLT (Night Light),
 *   FBL (Fireball), FIG (Figure/Humanoid), PRB (Probe), NFO (No Form Observable)
 *
 * Phenomena:
 *   WAV (Wave), TCH (Technical), HST (Historical), SND (Sound), ODD (Odd/Atypical),
 *   MID (Misidentification), RAY (Ray/Beam), SIG (Signal), LND (Landing), SUB (Submersible),
 *   OBS (Observation), VEH (Vehicle affected), TRC (Traces), DRT (Dirt/Ground traces),
 *   VEG (Vegetation affected), PHT (Photo/Video), RDA (Radiation), BLD (Building affected),
 *   OID (Humanoid), NOC (Nocturnal), ANI (Animals affected), HUM (Humans affected), INJ (Injury)
 *
 * Locale types:
 *   Town & City, Rural, Mountains, Farmlands, Coastal, Desert, Forest,
 *   Lake/River, Ocean, Airport, Military Base, Unknown
 */
export const observationsData = [
  {
    date: "2024-11-15",
    time: "22:30",
    location: "LAUSANNE, SUISSE",
    country: "Suisse",
    state: "Vaud",
    description:
      "Ce soir vers 22h30, j'ai observé depuis Ouchy un objet de forme triangulaire avec trois lumières blanches aux angles. L'objet se déplaçait silencieusement d'ouest en est à basse altitude. Plusieurs témoins dans la région ont rapporté des observations similaires la même nuit. La durée d'observation était d'environ 5 minutes avant qu'il ne disparaisse derrière les montagnes.",
    credibility: 8,
    strangeness: 7,
    duration: 300,
    locale: "Town & City",
    coordinates: { lat: 46.5197, lng: 6.6323 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["DLT"],
    phenomena: ["WAV", "OBS"],
    tags: ["triangle", "silencieux", "lumières-blanches", "vague"],
    imageFilename: "ovni-triangle-lausanne.jpg",
    userIndex: 0,
  },
  {
    date: "2024-10-28",
    time: "19:45",
    location: "GENÈVE, SUISSE",
    country: "Suisse",
    state: "Genève",
    description:
      "Observation depuis mon balcon. Une sphère orange très lumineuse est restée immobile pendant 10 minutes au-dessus du jet d'eau. J'ai pris plusieurs photos haute résolution avec mon appareil. La sphère émettait une lumière pulsante orange-rouge. Puis elle a disparu instantanément sans bruit.",
    credibility: 9,
    strangeness: 6,
    duration: 600,
    locale: "Town & City",
    coordinates: { lat: 46.2044, lng: 6.1432 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["PHT", "OBS"],
    tags: ["photo", "sphère", "orange", "immobile"],
    imageFilename: "ovni-sphere-geneve.jpg",
    userIndex: 1,
  },
  {
    date: "2024-09-12",
    time: "23:15",
    location: "BERNE, SUISSE",
    country: "Suisse",
    state: "Berne",
    description:
      "En rentrant à Berne, j'ai vu 7 points lumineux formant un V parfait. Intrigué, j'ai suivi la formation en voiture pendant 15 minutes. Ils se déplaçaient tous ensemble à la même vitesse, gardant exactement la même formation. J'ai dû arrêter car ils s'éloignaient trop vite vers le nord.",
    credibility: 7,
    strangeness: 8,
    duration: 900,
    locale: "Town & City",
    coordinates: { lat: 46.948, lng: 7.4474 },
    observerTypes: ["CIV"],
    ufoShapes: ["NLT"],
    phenomena: ["OBS", "VEH"],
    tags: ["formation", "poursuite", "sept-objets", "V-shape"],
    imageFilename: "ovni-formation-berne.jpg",
    userIndex: 2,
  },
  {
    date: "2024-08-05",
    time: "14:20",
    location: "ZÜRICH, SUISSE",
    country: "Suisse",
    state: "Zürich",
    description:
      "Observation en plein après-midi ! Avec mes jumelles, j'ai aperçu un objet discoïdal argenté. J'ai pu distinguer une structure en forme de dôme sur le dessus, des panneaux sur la surface inférieure. L'objet effectuait des montées verticales, arrêts brusques, virages à 90° à très haute vitesse. Diamètre estimé : 15-20 mètres.",
    credibility: 10,
    strangeness: 9,
    duration: 480,
    locale: "Town & City",
    coordinates: { lat: 47.3769, lng: 8.5417 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["TCH", "OBS"],
    tags: ["disque", "détails-structurels", "métallique", "dôme", "manœuvres"],
    imageFilename: "ovni-disque-zurich.jpg",
    userIndex: 3,
  },
  {
    date: "2024-07-20",
    time: "03:30",
    location: "LAC LÉMAN, SUISSE",
    country: "Suisse",
    state: "Vaud",
    description:
      "Incroyable ! Depuis la rive, j'ai observé une perturbation de l'eau, puis un objet cylindrique gris métallique est sorti lentement du lac. L'objet mesurait environ 10 mètres et émettait une lumière bleu-vert sous-marine. Une fois en surface, il est resté immobile 30 secondes, puis s'est élevé verticalement à une vitesse folle.",
    credibility: 8,
    strangeness: 10,
    duration: 180,
    locale: "Lake/River",
    coordinates: { lat: 46.452, lng: 6.595 },
    observerTypes: ["CIV"],
    ufoShapes: ["CIG"],
    phenomena: ["SUB", "OBS"],
    tags: ["submersible", "cylindre", "lac", "émersion", "lumière-bleue"],
    imageFilename: "ovni-submersible-leman.jpg",
    userIndex: 4,
  },
  {
    date: "2024-06-15",
    time: "21:45",
    location: "LYON, FRANCE",
    country: "France",
    state: "Rhône",
    description:
      "Observation depuis la place Bellecour à Lyon. Un disque argenté planait à 200m d'altitude et émettait un rayon lumineux blanc très intense qui balayait le sol comme un projecteur. Le rayon était parfaitement défini, comme un cylindre de lumière solide. J'ai ressenti une chaleur intense quand le rayon est passé près de moi.",
    credibility: 7,
    strangeness: 8,
    duration: 420,
    locale: "Town & City",
    coordinates: { lat: 45.764, lng: 4.8357 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["SCR"],
    phenomena: ["RAY", "OBS", "HUM"],
    tags: ["rayon", "projecteur", "balayage", "chaleur"],
    imageFilename: "ovni-rayon-lyon.jpg",
    userIndex: 5,
  },
  {
    date: "2024-05-22",
    time: "00:30",
    location: "PARIS, FRANCE",
    country: "France",
    state: "Île-de-France",
    description:
      "Depuis mon appartement parisien, j'ai observé un objet allongé de couleur gris métallique. Ce qui était troublant : un bourdonnement grave et modulé, presque musical. J'ai réussi à l'enregistrer avec mon téléphone. Le son était audible malgré la distance. L'objet planait 6 minutes, puis a accéléré brusquement. Le son s'est arrêté instantanément.",
    credibility: 9,
    strangeness: 7,
    duration: 360,
    locale: "Town & City",
    coordinates: { lat: 48.8566, lng: 2.3522 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["CIG"],
    phenomena: ["SND", "OBS"],
    tags: ["cigare", "son", "bourdonnement", "enregistrement"],
    imageFilename: "ovni-cigare-paris.jpg",
    userIndex: 6,
  },
  {
    date: "2024-04-18",
    time: "22:00",
    location: "MARSEILLE, FRANCE",
    country: "France",
    state: "Bouches-du-Rhône",
    description:
      "Expérience fascinante ! Vu depuis le Vieux-Port de Marseille, un objet avec lumières rouge, verte et blanche. Par curiosité, j'ai fait des signaux avec ma lampe torche (3 flashs rapides). L'objet a RÉPONDU avec le même pattern ! J'ai répété avec différents patterns : il répondait à chaque fois. C'était clairement une communication intentionnelle.",
    credibility: 8,
    strangeness: 9,
    duration: 900,
    locale: "Coastal",
    coordinates: { lat: 43.2965, lng: 5.3698 },
    observerTypes: ["CIV"],
    ufoShapes: ["NLT"],
    phenomena: ["SIG", "OBS"],
    tags: ["communication", "signaux", "réponse", "interaction"],
    imageFilename: "ovni-signaux-marseille.jpg",
    userIndex: 7,
  },
  {
    date: "2024-03-10",
    time: "04:15",
    location: "ALPES SUISSES",
    country: "Suisse",
    state: "Valais",
    description:
      "Randonnée dans les Alpes suisses. J'ai découvert un objet discoïdal posé dans une clairière. Diamètre 8 mètres, hauteur 3 mètres, couleur gris-bleu métallique. Trois pieds télescopiques le maintenaient au sol. L'objet a émis un sifflement aigu, les pieds se sont rétractés, et il s'est élevé verticalement. Au sol : trois marques circulaires de 40cm avec l'herbe brûlée en cercle de 6m.",
    credibility: 10,
    strangeness: 10,
    duration: 240,
    locale: "Mountains",
    coordinates: { lat: 46.0207, lng: 7.6586 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["LND", "TRC", "DRT", "VEG"],
    tags: ["atterrissage", "trace-sol", "disque", "pieds", "herbe-brûlée"],
    imageFilename: "ovni-atterrissage-alpes.jpg",
    userIndex: 8,
  },
  {
    date: "2024-02-28",
    time: "23:00",
    location: "TOULOUSE, FRANCE",
    country: "France",
    state: "Haute-Garonne",
    description:
      "Route de campagne près de Toulouse, 23h. Un objet triangulaire s'est positionné au-dessus de ma voiture. TOUT s'est arrêté instantanément : moteur, lumières, radio, téléphone. L'objet émettait une lumière orange pulsante. Je suis resté bloqué 5 minutes. Puis l'objet est parti à vitesse folle. Tout s'est rallumé immédiatement. Mon mécanicien n'a trouvé aucune anomalie.",
    credibility: 9,
    strangeness: 9,
    duration: 300,
    locale: "Rural",
    coordinates: { lat: 43.6047, lng: 1.4442 },
    observerTypes: ["CIV"],
    ufoShapes: ["DLT"],
    phenomena: ["VEH", "OBS"],
    tags: ["panne-électrique", "véhicule", "EME", "triangle"],
    imageFilename: "ovni-panne-toulouse.jpg",
    userIndex: 9,
  },
  {
    date: "2024-01-15",
    time: "04:00",
    location: "BORDEAUX, FRANCE",
    country: "France",
    state: "Gironde",
    description:
      "Ferme près de Bordeaux. Vers 4h du matin, tous les animaux se sont mis à paniquer. Les vaches couraient en cercle en beuglant, les chiens hurlaient. J'ai vu un disque lumineux orange planer au-dessus du pré. Les vaches étaient terrorisées. Après 10 minutes, l'objet est parti. Les animaux sont restés nerveux pendant 2 jours. Trois vaches ont refusé de produire du lait pendant une semaine.",
    credibility: 8,
    strangeness: 8,
    duration: 600,
    locale: "Farmlands",
    coordinates: { lat: 44.8378, lng: -0.5792 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["SCR"],
    phenomena: ["ANI", "OBS"],
    tags: ["animaux", "vaches", "panique", "ferme"],
    imageFilename: "ovni-animaux-bordeaux.jpg",
    userIndex: 0,
  },
  {
    date: "2023-12-20",
    time: "22:00",
    location: "STRASBOURG, FRANCE",
    country: "France",
    state: "Bas-Rhin",
    description:
      "Strasbourg, retour à pied vers 22h. J'ai vu une lumière aveuglante descendre vers moi. Soudain, impossible de bouger, comme paralysé. Puis trou noir. Je me suis retrouvé 2 heures plus tard à 5km de là, sans aucun souvenir. J'avais une marque rouge circulaire de 3cm sur l'avant-bras qui a mis 2 semaines à disparaître. Mal de tête pendant 3 jours.",
    credibility: 7,
    strangeness: 10,
    duration: 7200,
    locale: "Town & City",
    coordinates: { lat: 48.5734, lng: 7.7521 },
    observerTypes: ["CIV"],
    ufoShapes: ["NLT"],
    phenomena: ["HUM", "INJ", "OBS"],
    tags: ["paralysie", "temps-manquant", "marque", "enlèvement"],
    imageFilename: "ovni-enlevement-strasbourg.jpg",
    userIndex: 1,
  },
  {
    date: "2023-11-05",
    time: "10:30",
    location: "LUCERNE, SUISSE",
    country: "Suisse",
    state: "Lucerne",
    description:
      "Randonnée en forêt près de Lucerne. J'ai découvert un cercle parfait de 12 mètres où toute la végétation était carbonisée. Au centre, trois dépressions triangulaires de 60cm, profondes de 15cm, formant un triangle équilatéral de 4m de côté. Le sol était vitrifié par endroits. Pas de trace de feu normal. La veille, des témoins ont vu des lumières dans cette zone.",
    credibility: 9,
    strangeness: 9,
    duration: 0,
    locale: "Forest",
    coordinates: { lat: 47.0502, lng: 8.3093 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: [],
    phenomena: ["TRC", "DRT", "VEG", "LND"],
    tags: ["traces", "cercle", "brûlures", "sol-vitrifié"],
    imageFilename: "ovni-traces-lucerne.jpg",
    userIndex: 2,
  },
  {
    date: "2023-10-12",
    time: "21:30",
    location: "MONTREUX, SUISSE",
    country: "Suisse",
    state: "Vaud",
    description:
      "Montreux, observation d'un disque lumineux. Par chance, j'avais mon compteur Geiger (je suis physicien). Quand l'objet est passé au-dessus de moi à 100m, le compteur s'est affolé : niveau de radiation 50 fois supérieur à la normale ! Principalement des rayons gamma. L'objet est resté 5 minutes. Les niveaux sont revenus à la normale immédiatement. J'ai les enregistrements.",
    credibility: 10,
    strangeness: 9,
    duration: 300,
    locale: "Town & City",
    coordinates: { lat: 46.4312, lng: 6.9111 },
    observerTypes: ["SCI", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["RDA", "OBS"],
    tags: ["radiation", "geiger", "rayons-gamma", "scientifique", "mesures"],
    imageFilename: "ovni-radiation-montreux.jpg",
    userIndex: 3,
  },
  {
    date: "2023-09-28",
    time: "23:45",
    location: "NEUCHÂTEL, SUISSE",
    country: "Suisse",
    state: "Neuchâtel",
    description:
      "Observation vraiment étrange à Neuchâtel. Ce n'était pas un objet solide mais une entité lumineuse qui changeait constamment de forme : sphère, puis triangle, puis forme amorphe. Elle émettait des couleurs impossibles à décrire, entre le violet et une couleur inconnue. L'observation m'a laissé dans un état second, presque hypnotique. Sensation de communication télépathique.",
    credibility: 6,
    strangeness: 10,
    duration: 720,
    locale: "Town & City",
    coordinates: { lat: 46.992, lng: 6.9306 },
    observerTypes: ["CIV"],
    ufoShapes: ["NFO"],
    phenomena: ["ODD", "HUM"],
    tags: ["paranormal", "forme-changeante", "entité", "télépathie"],
    imageFilename: "ovni-entite-neuchatel.jpg",
    userIndex: 4,
  },
  {
    date: "1985-07-14",
    time: "22:30",
    location: "FRIBOURG, SUISSE",
    country: "Suisse",
    state: "Fribourg",
    description:
      "Je témoigne d'un événement vécu il y a 40 ans à Fribourg. J'avais 8 ans. Depuis la fenêtre de ma chambre, j'ai vu un énorme objet cylindrique avec des hublots lumineux passer lentement au-dessus de la ville. Mon père l'a vu aussi. Le lendemain : 5 familles l'avaient observé. Jamais d'explication officielle. Cette observation a changé ma vie.",
    credibility: 7,
    strangeness: 8,
    duration: 480,
    locale: "Town & City",
    coordinates: { lat: 46.806, lng: 7.1512 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["CIG"],
    phenomena: ["HST", "OBS", "WAV"],
    tags: ["historique", "1985", "enfance", "cylindre", "hublots"],
    imageFilename: "ovni-historique-fribourg.jpg",
    userIndex: 5,
  },
  {
    date: "2023-08-15",
    time: "16:00",
    location: "NICE, FRANCE",
    country: "France",
    state: "Alpes-Maritimes",
    description:
      "Nice, observation extraordinaire. J'ai vu un objet sombre qui s'est soudain entouré d'une brume blanche, devenant identique à un nuage ! Mais je l'avais vu avant la transformation. Le \"nuage\" se déplaçait contre le vent et gardait une forme trop régulière. Après 15 minutes, la brume s'est dissipée instantanément et l'objet métallique est réapparu avant de partir à grande vitesse.",
    credibility: 8,
    strangeness: 9,
    duration: 900,
    locale: "Coastal",
    coordinates: { lat: 43.7102, lng: 7.2619 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["NFO", "SCR"],
    phenomena: ["ODD", "TCH"],
    tags: ["camouflage", "nuage", "déguisement", "métamorphose"],
    imageFilename: "ovni-camouflage-nice.jpg",
    userIndex: 6,
  },
  {
    date: "2023-07-22",
    time: "02:15",
    location: "LILLE, FRANCE",
    country: "France",
    state: "Nord",
    description:
      "Observation de Lille. J'ai vu une longue file de lumières blanches se déplaçant en ligne droite d'ouest en est. Environ 40 points lumineux espacés régulièrement. Après recherche, il s'agissait probablement de satellites Starlink récemment lancés. Mais sur le moment, c'était impressionnant ! Je partage car d'autres pourraient voir la même chose.",
    credibility: 10,
    strangeness: 2,
    duration: 180,
    locale: "Town & City",
    coordinates: { lat: 50.6292, lng: 3.0586 },
    observerTypes: ["CIV"],
    ufoShapes: ["NLT"],
    phenomena: ["MID"],
    tags: ["satellites", "starlink", "identification", "ligne"],
    imageFilename: "satellites-starlink-lille.jpg",
    userIndex: 7,
  },
  {
    date: "2023-06-10",
    time: "01:00",
    location: "GRENOBLE, FRANCE",
    country: "France",
    state: "Isère",
    description:
      "Expérience bouleversante à Grenoble. Un disque lumineux s'est approché de ma maison. J'ai ressenti une présence dans mon esprit. Des pensées qui n'étaient pas les miennes, des images mentales de lieux inconnus. Message clair : \"Nous observons. Pas de danger. Évolution nécessaire.\" La communication a duré 10 minutes. Depuis, je reçois des \"flashs\" d'information. Suivi médicalement, pas de problème psychiatrique.",
    credibility: 5,
    strangeness: 10,
    duration: 600,
    locale: "Town & City",
    coordinates: { lat: 45.1885, lng: 5.7245 },
    observerTypes: ["CIV"],
    ufoShapes: ["SCR"],
    phenomena: ["HUM", "SIG", "ODD"],
    tags: ["contact", "télépathie", "message", "communication"],
    imageFilename: "ovni-contact-grenoble.jpg",
    userIndex: 8,
  },
  {
    date: "2023-05-05",
    time: "02:00",
    location: "NANTES, FRANCE",
    country: "France",
    state: "Loire-Atlantique",
    description:
      "Campagne près de Nantes, 2h du matin. Un disque était posé dans un champ. À côté, j'ai clairement vu deux silhouettes. Taille environ 1m20, tête disproportionnée, membres fins. Ils semblaient inspecter le sol. Quand ils m'ont vu, ils sont retournés vers l'objet qui est parti immédiatement. Tout a duré 2 minutes. J'étais pétrifié. Je n'en ai parlé à personne pendant 6 mois.",
    credibility: 6,
    strangeness: 10,
    duration: 120,
    locale: "Farmlands",
    coordinates: { lat: 47.2184, lng: -1.5534 },
    observerTypes: ["CIV"],
    ufoShapes: ["SCR", "FIG"],
    phenomena: ["OID", "LND"],
    tags: ["humanoïde", "entité", "petits-gris", "rencontre"],
    imageFilename: "ovni-humanoide-nantes.jpg",
    userIndex: 9,
  },
  {
    date: "2023-04-12",
    time: "21:00",
    location: "DIJON, FRANCE",
    country: "France",
    state: "Côte-d'Or",
    description:
      'Observation massive près de Dijon. Plus de 50 témoins ont vu un énorme triangle. Le lendemain, des militaires ont bouclé la zone. Ils ont interrogé tous les témoins et demandé de ne rien dire. Un hélicoptère militaire a survolé la zone pendant 2 jours. Les journaux locaux n\'ont rien publié malgré les témoignages. Réponse officielle : "exercice militaire". Mais nous savons ce que nous avons vu.',
    credibility: 9,
    strangeness: 8,
    duration: 1200,
    locale: "Town & City",
    coordinates: { lat: 47.322, lng: 5.0415 },
    observerTypes: ["CIV", "MIL", "NWS"],
    ufoShapes: ["DLT"],
    phenomena: ["WAV", "OBS"],
    tags: [
      "dissimulation",
      "militaires",
      "cover-up",
      "censure",
      "témoins-multiples",
    ],
    imageFilename: "ovni-coverup-dijon.jpg",
    userIndex: 0,
  },
  {
    date: "2023-03-18",
    time: "20:30",
    location: "ANNECY, FRANCE",
    country: "France",
    state: "Haute-Savoie",
    description:
      "Observation à Annecy. J'ai fait mon devoir de citoyen et déposé un rapport complet à la gendarmerie. Ils ont été professionnels et ont pris ma déposition au sérieux. Le gendarme m'a dit qu'il y avait eu 3 autres témoignages la même nuit dans la région. Il m'a donné un numéro de dossier et dit que ça serait transmis au GEIPAN (organisme officiel français d'étude des PAN).",
    credibility: 8,
    strangeness: 6,
    duration: 480,
    locale: "Town & City",
    coordinates: { lat: 45.8992, lng: 6.1294 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["NLT"],
    phenomena: ["OBS"],
    tags: ["rapport-officiel", "gendarmerie", "GEIPAN"],
    imageFilename: "ovni-rapport-annecy.jpg",
    userIndex: 1,
  },
  {
    date: "2023-02-25",
    time: "08:00",
    location: "VOSGES, FRANCE",
    country: "France",
    state: "Vosges",
    description:
      "Forêt des Vosges. Découverte troublante : des empreintes de \"pas\" de 25cm avec seulement 3 orteils. Profondeur de 4cm dans la terre. Une piste de 50 mètres menant à une zone circulaire aplatie de 8m où l'herbe était couchée en spirale. Moulages réalisés. Un biologiste les a examinées : pas d'animal connu. Corrélation : lumières vues dans le ciel la nuit précédente.",
    credibility: 9,
    strangeness: 9,
    duration: 0,
    locale: "Forest",
    coordinates: { lat: 48.2081, lng: 7.0522 },
    observerTypes: ["CIV", "SCI"],
    ufoShapes: [],
    phenomena: ["TRC", "DRT", "OID"],
    tags: ["empreintes", "traces", "pas", "sol", "moulage"],
    imageFilename: "traces-vosges.jpg",
    userIndex: 2,
  },
  {
    date: "2023-01-10",
    time: "07:30",
    location: "COLMAR, FRANCE",
    country: "France",
    state: "Haut-Rhin",
    description:
      "Agriculteur près de Colmar. Découvert un cercle parfait de 15m de diamètre dans mon champ de blé. Les tiges n'étaient pas cassées mais pliées à 90° au niveau des nœuds. Impossible à reproduire manuellement sans casser. Au centre, un motif géométrique complexe. Les grains dans le cercle germaient 3 fois plus vite. Analyse d'un agronome : inexplicable. Aucun signe de fraude.",
    credibility: 9,
    strangeness: 8,
    duration: 0,
    locale: "Farmlands",
    coordinates: { lat: 48.0779, lng: 7.3589 },
    observerTypes: ["CIV", "SCI"],
    ufoShapes: [],
    phenomena: ["VEG", "TRC"],
    tags: ["crop-circle", "blé", "géométrique", "plantes"],
    imageFilename: "cercle-colmar.jpg",
    userIndex: 3,
  },
  {
    date: "2022-12-15",
    time: "23:45",
    location: "MONTPELLIER, FRANCE",
    country: "France",
    state: "Hérault",
    description:
      "Quartier de Montpellier, 23h45. Un objet lumineux planait au-dessus du transformateur électrique. Soudain, TOUT le quartier (500 maisons) s'est retrouvé dans le noir total. L'objet émettait une lumière bleue pulsante. Après 20 minutes, il est parti verticalement. L'électricité est revenue 2 minutes après. EDF n'a trouvé aucune anomalie technique. Plusieurs témoins.",
    credibility: 8,
    strangeness: 8,
    duration: 1200,
    locale: "Town & City",
    coordinates: { lat: 43.6108, lng: 3.8767 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["NLT"],
    phenomena: ["BLD", "VEH"],
    tags: ["panne", "électricité", "transformateur", "quartier"],
    imageFilename: "panne-montpellier.jpg",
    userIndex: 4,
  },
  {
    date: "2022-11-20",
    time: "15:00",
    location: "VAUD, SUISSE",
    country: "Suisse",
    state: "Vaud",
    description:
      "Vaud, Suisse. Approché d'un objet atterri à moins de 10 mètres. Sensation de chaleur intense. Le lendemain, brûlures au 1er degré sur le visage et les mains, comme un coup de soleil sévère. Nausées pendant 48h. Analyse médicale : brûlures compatibles avec exposition aux UV intenses ou radiations. Impossible naturellement vu la météo. Arrêt maladie de 1 semaine.",
    credibility: 8,
    strangeness: 9,
    duration: 180,
    locale: "Rural",
    coordinates: { lat: 46.5197, lng: 6.6335 },
    observerTypes: ["CIV"],
    ufoShapes: ["SCR"],
    phenomena: ["INJ", "RDA", "LND"],
    tags: ["brûlures", "radiation", "blessure", "médical"],
    imageFilename: "brulures-vaud.jpg",
    userIndex: 5,
  },
  {
    date: "2022-10-08",
    time: "02:30",
    location: "JURA, SUISSE",
    country: "Suisse",
    state: "Jura",
    description:
      "Village du Jura suisse. Depuis 3 semaines, chaque nuit entre 2h et 3h du matin, des lumières oranges apparaissent au-dessus de la montagne. Parfois une seule, parfois jusqu'à 5. Elles restent stationnaires 10-15 minutes puis disparaissent. Toute la communauté en parle. Nous avons établi un planning de veille. Phénomène régulier et prévisible. Aucune entité visible.",
    credibility: 9,
    strangeness: 7,
    duration: 900,
    locale: "Mountains",
    coordinates: { lat: 47.0598, lng: 6.7658 },
    observerTypes: ["CIV", "GND"],
    ufoShapes: ["NLT"],
    phenomena: ["NOC", "WAV"],
    tags: ["récurrent", "nocturne", "régulier", "communauté"],
    imageFilename: "nocturne-jura.jpg",
    userIndex: 6,
  },
  {
    date: "2022-09-15",
    time: "21:00",
    location: "BESANÇON, FRANCE",
    country: "France",
    state: "Doubs",
    description:
      "Besançon, 21h. Un triangle noir immense (estimé 100m d'envergure) a survolé la ville à très basse altitude, environ 150m. Complètement silencieux ! Trois lumières blanches aux angles, une rouge au centre. Vol très lent, 30 km/h maximum. Observation de 8 minutes. Des dizaines de témoins dans les rues. Pas d'explication officielle. Partie d'une vague d'observations cette semaine.",
    credibility: 9,
    strangeness: 9,
    duration: 480,
    locale: "Town & City",
    coordinates: { lat: 47.238, lng: 6.024 },
    observerTypes: ["CIV", "GND", "NWS"],
    ufoShapes: ["DLT"],
    phenomena: ["WAV", "OBS"],
    tags: ["triangle", "géant", "silencieux", "ville", "vague"],
    imageFilename: "triangle-besancon.jpg",
    userIndex: 7,
  },
  {
    date: "2022-08-22",
    time: "20:15",
    location: "GENÈVE, SUISSE",
    country: "Suisse",
    state: "Genève",
    description:
      "Genève, parc des Bastions. Sphère lumineuse changeant de couleur de façon régulière : blanc -> bleu -> vert -> jaune -> rouge, puis recommence. Cycle de 10 secondes exactement. Objet stationnaire pendant 25 minutes. J'ai pris une série de photos montrant la séquence complète. Puis départ fulgurant vers le nord-est. J'ai chronométré tout avec précision.",
    credibility: 9,
    strangeness: 7,
    duration: 1500,
    locale: "Town & City",
    coordinates: { lat: 46.2017, lng: 6.1428 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["PHT", "OBS"],
    tags: ["photo", "couleurs", "cyclique", "sphère"],
    imageFilename: "sphere-couleurs-geneve.jpg",
    userIndex: 8,
  },
  {
    date: "2022-07-30",
    time: "22:45",
    location: "BÂLE, SUISSE",
    country: "Suisse",
    state: "Bâle-Ville",
    description:
      "Bâle, observation fascinante de deux disques argentés. Ils émettaient des flashs lumineux l'un vers l'autre, comme un échange de signaux. Pattern complexe : 3 flashs courts, pause, 2 flashs longs, etc. Cet échange a duré 10 minutes. Puis ils sont partis ensemble dans la même direction, en formation serrée. Communication évidente entre les deux objets.",
    credibility: 8,
    strangeness: 8,
    duration: 600,
    locale: "Town & City",
    coordinates: { lat: 47.5596, lng: 7.5886 },
    observerTypes: ["CIV", "HQO"],
    ufoShapes: ["SCR"],
    phenomena: ["SIG", "OBS"],
    tags: ["deux-objets", "communication", "flashs", "disques"],
    imageFilename: "duo-bale.jpg",
    userIndex: 9,
  },
];

export default observationsData;
