...
<template>
  <div class="w-full">
    ...
    <div v-if="label || showValue" class="flex items-center justify-between mb-2">
      <label v-if="label" class="text-xs uppercase tracking-wider text-white/60">
        {{ label }}
      </label>
...
.range-input {
  ...
  appearance: none;
  height: 4px;
  background: linear-gradient(to right, #00F0FF var(--progress), rgba(255, 255, 255, 0.1) var(--progress));
  border-radius: 2px;
  cursor: pointer;
}
