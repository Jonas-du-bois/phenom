    ...
    <Transition name="fade">
      ...
      >
        <div class="flex items-center justify-between px-4 py-2 border-b border-white/10">
          <span class="text-xs text-white/40 uppercase tracking-wider">Récentes</span>
          <button 
            @click="clearRecent"
            class="text-xs text-[#00F0FF]"
          >
            Effacer
          </button>
        </div>
...
const emit = defineEmits(['update:modelValue', 'search', 'clear', 'clear-recent'])
...
const showSuggestions = computed(() => isFocused.value && query.value.length >= 2)
