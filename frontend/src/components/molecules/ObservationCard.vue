...
const relativeDate = computed(() => {
  return formatRelativeTime(props.observation.createdAt || props.observation.date, true)
})
...
const userName = computed(() => {
  return props.observation.userId?.name || props.observation.user?.name || 'Anonyme'
})

// Avatar de l'utilisateur
const userAvatar = computed(() => {
  return props.observation.userId?.avatar || props.observation.user?.avatar || ''
})

// Nombre de commentaires (API renvoie commentsCount ou comments array)
const commentCount = computed(() => {
  const base = props.observation.commentsCount 
...
const submitComment = async () => {
  ...
  try {
    const newComment = await commentStore.addComment(props.observation._id, commentText.value)
...
<template>
  <article 
    class="bg-[#000000] border-b border-white/10"
  ...
  >
    ...
    <div class="flex items-center gap-3 px-4 py-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <span class="text-sm font-medium text-white truncate">{{ userName }}</span>
          <span class="text-white/30">•</span>
          <span class="text-xs text-white/50">{{ relativeDate }}</span>
        </div>
    ...
    <div 
      v-if="mainImage" 
      class="relative aspect-[4/3] bg-[#12151C] overflow-hidden"
    >
      <img 
        :src="mainImage" 
    ...
    >
      <svg class="w-16 h-16 text-white/10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1">
    ...
    <div class="px-4 py-3 space-y-3">
      ...
      <!-- Durée -->
      <div v-if="observation.duration" class="flex items-center gap-2 text-xs text-white/50">
      ...
      <Transition name="slide">
        <div v-if="showComments" class="mt-3 space-y-3" @click="stopPropagation">
          <!-- Loading -->
          <div v-if="isLoadingComments" class="flex justify-center py-4">
            <div class="w-5 h-5 border-2 border-[#00F0FF]/30 border-t-[#00F0FF] rounded-full animate-spin"></div>
          </div>
          
          <!-- Liste des commentaires -->
          <div v-else-if="comments.length > 0" class="space-y-3 max-h-60 overflow-y-auto">
            <div 
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-white/40">{{ formatRelativeTime(comment.createdAt, true) }}</span>
                </div>
                <p class="text-sm text-white/70 mt-1">{{ comment.content }}</p>
              </div>
            </div>
          </div>
    ...
    <!-- Actions Bar -->
    <div class="flex items-center justify-around px-4 py-3 border-t border-white/5">
      <!-- Commenter -->
      <button 
