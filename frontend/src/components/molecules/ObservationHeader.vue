...
const formattedDate = computed(() => {
  const dateValue = props.date || props.observation?.date || props.observation?.createdAt
  if (!dateValue) return ''
...
<template>
  <header class="sticky top-0 z-20 flex items-center justify-between px-4 mt-2 py-3 bg-[#000000] border-b border-white/10 safe-area-top">
    <!-- Back Button -->
    <button 
      v-if="showBack"
