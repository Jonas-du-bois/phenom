...
export const validateDate = (date, options = {}) => {
  ...
  if (minDate && dateObj < new Date(minDate)) {
    return { valid: false, error: `${fieldName} ne peut pas être antérieure au ${new Date(minDate).toLocaleDateString('fr-FR')}` }
  }

  if (maxDate && dateObj > new Date(maxDate)) {
    return { valid: false, error: `${fieldName} ne peut pas être postérieure au ${new Date(maxDate).toLocaleDateString('fr-FR')}` }
  }
