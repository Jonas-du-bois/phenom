...
<template>
  <AppLayout :show-tab-bar="false">
    <template #header>
      <PageHeader title="Administration" show-back back-to="/settings" />
    </template>
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex gap-3 p-3">
                <div
                  class="w-16 h-16 rounded-lg overflow-hidden bg-white/5 shrink-0"
                >
                  <img
                    v-if="obs.imageUrl"
                    :src="obs.imageUrl"
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-2 mt-1">
                    <BaseBadge v-if="obs.verified" variant="success" size="sm"
                      >Vérifié</BaseBadge
                    >
                    <BaseBadge v-else variant="warning" size="sm"
                      >En attente</BaseBadge
                    >
                  </div>
                </div>
              </div>
...
watch(activeTab, (tab) => {
  ...
  if (tab === "observations" && !adminObservations.value.length)
    fetchObservations();
