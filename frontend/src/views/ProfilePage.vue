      ...
      <template v-else-if="user">
        ...
        <div class="px-4 py-6 space-y-6">
          ...
          <div class="flex flex-col items-center text-center">
            ...
            <p v-if="user.bio" class="text-white/60 text-sm mt-2 max-w-sm">{{ user.bio }}</p>
            <p v-if="user.location" class="text-white/40 text-sm mt-2 flex items-center gap-1">
          ...
          <div class="grid grid-cols-3 gap-4 py-4 border-y border-white/10">
            <div class="text-center">
              <div class="text-2xl font-bold text-white">{{ stats.observations }}</div>
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-white/40 uppercase tracking-wider mt-1">Commentaires</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-[#00F0FF]">{{ averageCredibility }}</div>
              <div class="text-xs text-white/40 uppercase tracking-wider mt-1">Créd. moy.</div>
            </div>
          </div>
          ...
          <template v-if="activeTab === 'observations'">
            ...
            >
              <EmptyState
                ...
                :description="isOwnProfile ? 'Créez votre première observation !' : 'Cet utilisateur n\'a pas encore d\'observations.'"
              >
...
import { IconButton, BaseButton, BaseAvatar, LoadingSpinner, ErrorState, EmptyState } from '@/components/atoms'
...
const averageCredibility = computed(() => {
  ...
  const sum = observations.value.reduce((acc, o) => acc + (o.credibilityScore || 0), 0)
  return (sum / observations.value.length).toFixed(1)
})
...
const fetchProfile = async () => {
  ...
  try {
    ...
    // Fetch user observations via store
    await observationStore.fetchObservations({ userId, limit: 50 })
    
    // Update stats
    stats.value.observations = observations.value.length
